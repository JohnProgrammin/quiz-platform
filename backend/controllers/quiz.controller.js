...
/**
 * Generate mastery quiz from a previous attempt's weak topics
 * Creates a focused 5-question quiz on topics where user scored poorly
 */
exports.generateMasteryQuiz = async (req, res) => {
  try {
    ...
    const { data: attempt, error: attemptError } = await supabase
      .from('quiz_attempts')
      .select('id, quiz_id, percentage, answers, completed_at')
      .eq('id', attemptId)
      .eq('user_id', userId)
      .single();
    ...
    // Get the original quiz to understand topics
    const { data: quiz, error: quizError } = await supabase
      .from('quizzes')
      .select('id, title, note_id, questions')
    ...
    // Get the note to determine topic
    let noteTitle = 'General Review';
    if (quiz.note_id) {
    ...
    // Only generate if score was below 80% (needs improvement)
    if (attempt.percentage >= 80) {
      return res.status(400).json({
        error: 'Score is already high. Mastery quiz only for scores below 80%.',
      });
    }

    // Generate 5-question mastery quiz on this topic
    const masteryQuizId = uuidv4();
    const questions = await aiService.generateWeaknessMasteryQuiz(
      noteTitle,
    ...
    if (!questions || questions.length === 0) {
      return res.status(400).json({ error: 'Failed to generate mastery quiz questions' });
    }

    // Ensure each question has an ID
    questions.forEach((q, i) => {
      if (!q.id) q.id = `q_mastery_${i + 1}`;
    });

    // Insert the mastery quiz
    const { error: insertError } = await supabase
      .from('quizzes')
      .insert([{
        id: masteryQuizId,
        user_id: userId,
        title: `Mastery: ${noteTitle}`,
        created_at: new Date(),
        ...
        ai_generation_metadata: {
          ...
          parent_percentage: attempt.percentage,
          topic: noteTitle,
        },
        questions: questions,
      }]);
    ...
    res.status(201).json({
      id: masteryQuizId,
      title: `Mastery: ${noteTitle}`,
      description: `Focused quiz on ${noteTitle} to improve from ${attempt.percentage}%`,
      questionCount: questions.length,
      questions: questions.map((q) => ({
        id: q.id,
        text: q.text || q.question,
    ...
    console.error('Generate mastery quiz error:', error);
    res.status(500).json({ error: 'Failed to generate mastery quiz' });
  }
};
